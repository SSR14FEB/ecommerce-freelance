...
const updateProductMediaController = asyncHandler(async(req:Request,res:Response)=>{
    const media:UpdateProductMediaPayload = req.body
    media["files"] = req.files as any 

    const updatedProductMedia = await updateProductMedia(media)
    if(!updatedProductMedia){
        throw new ApiError(400,"Failed to update product media","")
    }
    return res.status(201)
    .json(new ApiResponse(201,"product is updated successfully",true,updatedProductMedia))
})
