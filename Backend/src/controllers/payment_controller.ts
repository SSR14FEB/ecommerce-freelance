...
const createPaymentIntentController = asyncHandler(
...
const verifyPaymentController = asyncHandler(
...
const handlePaymentWebhookController = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    return res
      .status(200)
      .json(
        new ApiResponse(200, "Webhook processed successfully", true, { result })
      );
  }
);

const refundPaymentController = asyncHandler(
  async (req: Request, res: Response) => {
    const { orderId, productId, quantity, reason } = req.params;
    if (
      !mongoose.Types.ObjectId.isValid(orderId as string) ||
    ...
    await refundPayment(
      orderId as string,
      productId as string,
      quantityNum as number,
      reason as string
    );
    return res
      .status(200)
      .json(new ApiResponse(200, "Payment refund processed", true));
  }
);

const refundStatusController = asyncHandler(
  async (req: Request, res: Response) => {
    const { refund_Id, payment_Id, user_Id } = req.params;
    if (
      !mongoose.Types.ObjectId.isValid(refund_Id as string) ||
      !mongoose.Types.ObjectId.isValid(payment_Id as string) ||
      !mongoose.Types.ObjectId.isValid(user_Id as string)
    ) {
      throw new ApiError(400, "Invalid payment id or refund id", "");
    }
    const status = await refundStatus(
      refund_Id as string,
    ...
    if (!status) {
      throw new ApiError(
        400,
        "Cant fetched status at the moment try after some time",
        ""
      );
    }
    return res
      .status(200)
      .json(new ApiResponse(200, "status fetched successfully", true, { status }));
  }
);
export {
